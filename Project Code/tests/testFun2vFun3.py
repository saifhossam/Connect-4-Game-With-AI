...
def score_position(board, piece):
	...
	for row in range(ROW_COUNT):
		...
		for col in range(COLUMN_COUNT-3):
			window = row_array[col:col+WINDOW_LENGTH2]
	...
	for col in range(COLUMN_COUNT):
		...
		for row in range(ROW_COUNT-3):
			window = col_array[row:row+WINDOW_LENGTH2]
			score += evaluate_window(window, piece)

	##  Diagonal
	for row in range(ROW_COUNT-3):
		for col in range(COLUMN_COUNT-3):
			window = [board[row+i][col+i] for i in range(WINDOW_LENGTH2)]
			score += evaluate_window(window, piece)

	for row in range(ROW_COUNT-3):
		for col in range(COLUMN_COUNT-3):
			window = [board[row+3-i][col+i] for i in range(WINDOW_LENGTH2)]
			score += evaluate_window(window, piece)
...
while not game_over:
	...
	if turn == AI and not game_over:	
		if counter2 == 0 or counter2 == 1:
			...
			col, minimax_score = minimax2(board, 3, -math.inf, math.inf, True)
